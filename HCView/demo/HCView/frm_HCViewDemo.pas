...
procedure TfrmHCViewDemo.SnapScreen(a, b, c, d: Integer; Apath: string); // 任意区域截图,参数为截图坐标
var
  bmpscreen: Tbitmap;
...
begin
  ...
  CopyScreenToBmp(SForm2.Image1); // 捕捉整屏图形到form2中的image1
  if SForm2.ShowModal = mrOk then // 在form2中进行图片裁剪
  begin
    WindowState := wsNormal; // 复原窗口状态
    show;
    FHCView.Paste;
  end
  else
...
begin
  case (Sender as TToolButton).Tag of
    ...
    3:
      FHCView.ApplyParaAlignHorz(TParaAlignHorz.pahJustify); // 两端
    4:
      FHCView.ApplyParaAlignHorz(TParaAlignHorz.pahScatter); // 分散
    5:
...
begin
  ...
  begin
    ...
    begin // 重新载入
      Caption := '文档管理' + '  ' + ExtractFileName(fName);
      vExt := LowerCase(ExtractFileExt(fName)); // 后缀
      if vExt = HC_EXT then
...
begin
  statbar.Panels[0].Text := '预览' + IntToStr(FHCView.PagePreviewFirst + 1) + '页 光标' + IntToStr(FHCView.ActivePageIndex + 1) + '页 共' + IntToStr(FHCView.PageCount) + '页';
end;
...
begin
  ...
  begin
    case (Sender as TMenuItem).Tag of
      0:
        FHCView.ApplyParaLineSpace(TParaLineSpaceMode.pls100); // 单倍
      1:
        FHCView.ApplyParaLineSpace(TParaLineSpaceMode.pls115); // 1.15倍
      2:
        FHCView.ApplyParaLineSpace(TParaLineSpaceMode.pls150); // 1.5倍
      3:
        FHCView.ApplyParaLineSpace(TParaLineSpaceMode.pls200); // 双倍
      4:
...
begin
  if SaveFile then
    statbar.Panels[1].Text := '保存成功' // ShowMessage('保存成功！');
  else
    statbar.Panels[1].Text := '保存失败';
end;
